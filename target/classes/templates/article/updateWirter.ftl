...
<html lang="zh-CN">
  <head>
    ...
    <script>
        $(function(){
            ...
            var freemake_text= "<#if article??>${article.text?js_string}</#if>"
            editor.create();
            editor.txt.html(freemake_text)
            $("#but").click(function(){
  ...
  <body>
    ...
    <h1>文章编写：</h1>
    <#if article??>
    标题<input type="text" name="title" id="title" value="${article.title}">
    关键字<input type="text" name="keyWord" id="keyWord" value="${article.keyWord}">
    标签<select id="isOriginal">
        <option disabled >--请选择是否原创--</option>
        <option value="0"
